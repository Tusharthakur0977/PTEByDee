        ...
        <div className="relative z-[201] flex w-full max-w-6xl max-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 text-left align-middle shadow-[0_30px_120px_rgba(15,23,42,0.22)] transition-all dark:border-slate-700/70 dark:bg-slate-950 dark:shadow-[0_30px_120px_rgba(15,23,42,0.55)]">
                  ...
                  <div className="space-y-3">
                    <div>
                      <h3 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">
                        Listening Fill In The Blanks Analysis
                      </h3>
                      <p className="mt-1 max-w-2xl text-sm text-slate-600 dark:text-slate-400">
                        Review your typed answers and identify spelling improvements.
                      </p>
                    </div>
          ...
          <div className="custom-scrollbar flex-1 overflow-y-auto p-4 sm:p-6">
            <div className="grid gap-5 xl:grid-cols-2">
              <div className="min-w-0 flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-[0_18px_60px_rgba(15,23,42,0.24)]">
                    ...
                    <div className="flex items-center gap-4">
                       <div>
                         <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Correct</p>
                         <p className="text-2xl font-bold text-emerald-500">{blankEntries.filter(b => b.isCorrect).length}</p>
                       </div>
                       <div className="h-8 w-px bg-slate-200 dark:bg-slate-700"></div>
                       <div>
                         <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Incorrect</p>
                         <p className="text-2xl font-bold text-rose-500">{blankEntries.filter(b => !b.isCorrect).length}</p>
                       </div>
                    </div>
                  </div>
                </div>
                ...
                {(response.textResponse || analysis.userText) && (
                  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-[0_18px_60px_rgba(15,23,42,0.24)]">
                    <div className="mb-4 flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-500/20 bg-slate-500/10 text-slate-600 dark:text-slate-300">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4 text-sm leading-relaxed text-slate-700 dark:border-slate-700/50 dark:bg-slate-950/50 dark:text-slate-300">
                      {response.textResponse || analysis.userText}
                    </div>
                  </div>
                )}
              ...
              <div className="min-w-0 flex flex-col gap-5">
                <div className="rounded-3xl border border-slate-200 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-[0_18px_60px_rgba(15,23,42,0.24)]">
                  ...
                  <div className="px-5 py-5 overflow-y-auto max-h-[500px] custom-scrollbar">
                    {blankEntries.length > 0 ? (
                        ...
                        {blankEntries.map((item) => (
                          <div
                            ...
                            className={`rounded-2xl border p-4 transition-all hover:shadow-md ${
                              item.isCorrect
                                ? "border-emerald-200 bg-emerald-50/50 dark:border-emerald-500/10 dark:bg-emerald-500/5"
                                : "border-rose-200 bg-rose-50/50 dark:border-rose-500/10 dark:bg-rose-500/5"
                            }`}
                          >
                            ...
                            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                              <div className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700/50 dark:bg-slate-950/50">
                      ...
                      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm text-slate-500 dark:border-slate-700/80 dark:bg-slate-950/40 dark:text-slate-400">
            ...
            {spellingErrors.length > 0 && (
              <div className="mt-5 rounded-3xl border border-rose-200 bg-rose-50/50 p-6 shadow-sm dark:border-rose-500/10 dark:bg-rose-500/5">
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-rose-700/70 dark:text-rose-400/70">
                      Review and learn from your spelling mistakes.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  {spellingErrors.map((error, index) => (
                    <div
                      key={index}
                      className="rounded-2xl border border-white bg-white/60 p-5 shadow-sm dark:border-slate-700/30 dark:bg-slate-900/40"
                    >
                      <div className="flex flex-col gap-3">
                         <div className="flex items-center justify-between border-b border-slate-100 pb-2 dark:border-slate-800">
                            <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Error {index + 1}</span>
                         </div>
                         <div className="grid grid-cols-2 gap-4">
                            <div>
                               <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Your Word</p>
                               <p className="text-sm font-semibold text-rose-500 mt-1">{error.text || "Unknown"}</p>
                            </div>
                            <div>
                               <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Correct</p>
                               <p className="text-sm font-semibold text-emerald-500 mt-1">{error.correction || "Unknown"}</p>
                            </div>
                         </div>
                         {error.explanation && (
                ...
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700/70 dark:text-amber-400/70">
                      Overall feedback on your listening comprehension.
                    </p>
                  </div>
                </div>
