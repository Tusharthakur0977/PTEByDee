...
const ResponseDetailModal: React.FC<ResponseDetailModalProps> = ({
  ...
  const blankResults = analysis.blankResults || analysis.itemResults || {};
        ...
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Your performance in this Reading FIB task.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-3">
            {scoreEntries.length > 0 ? (
                ...
                <div className="flex h-full flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 p-6 text-center text-slate-500 dark:border-slate-700 dark:text-slate-400">
                    <LayoutGrid className="mb-2 h-8 w-8 opacity-20" />
                    <p className="text-sm">Specific rubric scores not available.</p>
                </div>
            )}
          </div>
        </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="mt-1 max-w-2xl text-sm text-slate-600 dark:text-slate-400">
                        Evaluate your contextual word choices and review the correct vocabulary for each blank.
                      </p>
                    </div>
            ...
            <div className="grid gap-5 xl:grid-cols-2">
              ...
              <div className="min-w-0 flex flex-col gap-5">
                <div className="rounded-3xl border border-slate-200 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-[0_18px_60px_rgba(15,23,42,0.24)]">
                  ...
                  <div className="px-5 py-5 space-y-4 overflow-y-auto max-h-[500px] custom-scrollbar">
                    {Object.entries(blankResults).length > 0 ? (
                      Object.entries(blankResults).map(([blankKey, result]: [string, any]) => (
                        <div
                          key={blankKey}
                          className={`group relative rounded-2xl border p-4 transition-all ${
                            result.isCorrect
                              ? "border-emerald-200 bg-emerald-50/40 dark:border-emerald-500/10 dark:bg-emerald-500/5"
                              : "border-rose-200 bg-rose-50/40 dark:border-rose-500/10 dark:bg-rose-500/5"
                          }`}
                        >
                          <div className="flex items-center justify-between mb-3">
                            ...
                            {result.isCorrect ? (
                              <div className="flex items-center gap-1.5 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400">
                                <CheckCircle2 className="h-3 w-3" /> Correct
                              </div>
                            ) : (
                              <div className="flex items-center gap-1.5 rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-rose-700 dark:bg-rose-500/20 dark:text-rose-400">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <p className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">Your Answer</p>
                              <p className={`mt-1 text-sm font-semibold ${result.isCorrect ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300'}`}>
                                {result.userAnswer || "(Not Selected)"}
                              </p>
                            </div>
                            {!result.isCorrect && (
                              <div>
                                <p className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">Correct Answer</p>
                      ...
                      <div className="flex flex-col items-center justify-center py-10 text-center opacity-50">
                        ...
                        <p className="text-sm text-slate-500">No blank analysis data available.</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700/70 dark:text-amber-400/70">
                      Understanding the correct word choice based on grammar and syntax.
                    </p>
                  </div>
                </div>
