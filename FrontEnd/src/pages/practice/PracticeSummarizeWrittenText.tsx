...
const PracticeSummarizeWrittenText: React.FC = () => {
    ...
    prepTimerRef.current = setInterval(() => {
      setPrepTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          const writingDuration = currentQuestion?.content?.timeLimit || 600;
...
export default PracticeSummarizeWrittenText;
...
export interface DetailedAnalysis {
  ...
  maxFormScore: number;
  grammarScore: number;
  maxGrammarScore: number;
  vocabularyScore: number;
  maxVocabularyScore: number;
