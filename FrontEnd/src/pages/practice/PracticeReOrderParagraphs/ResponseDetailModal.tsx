...
const ResponseDetailModal: React.FC<ResponseDetailModalProps> = ({
  ...
  const paragraphCount = Math.max(
    userOrderText.length,
    correctOrderText.length,
  );
        ...
        {paragraphs.length > 0 ? (
          <div className="space-y-3">
            {paragraphs.map((paragraph, index) => (
              <div
                key={`${title}-${index}-${paragraph.slice(0, 24)}`}
                className="flex gap-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900/40"
              >
                <div
                  className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-sm font-semibold ${badgeClasses}`}
                >
                  {index + 1}
                </div>
          ...
          <div className="flex items-start justify-between gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-700">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                Reorder Paragraphs Response Analysis
              </h3>
            ...
            <div className="max-h-[70vh] space-y-6 overflow-y-auto overscroll-contain pr-1">
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                {renderParagraphList(
                  ...
                  "No paragraph order was recorded for this attempt.",
                )}
                ...
                {renderParagraphList(
                  ...
                  "Correct paragraph order is not available for this attempt.",
                  "success",
                )}
              </div>
