...
const ResponseDetailModal: React.FC<ResponseDetailModalProps> = ({
  ...
  const reorderResult = (analysis.reorderResult || {}) as ReorderResult;
  ...
  const userOrderText = normalizeParagraphs(reorderResult.userOrderText);
  const correctOrderText = normalizeParagraphs(reorderResult.correctOrderText);
  const correctPairs =
    typeof reorderResult.correctPairs === "number"
      ? reorderResult.correctPairs
      : 0;
  const maxPairs =
    typeof reorderResult.maxPairs === "number"
      ? reorderResult.maxPairs
      : Math.max(correctOrderText.length - 1, 0);

  const explanationParagraphs =
    typeof reorderResult.explanation === "string"
            ...
            <div className="grid gap-5 xl:grid-cols-2">
              <div className="min-w-0 flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-[0_18px_60px_rgba(15,23,42,0.24)]">
                    ...
                    <p className="text-3xl font-bold text-slate-900 dark:text-slate-100">
                      {correctPairs}
                      <span className="ml-1 text-base font-medium text-slate-500 dark:text-slate-400">/ {maxPairs}</span>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="min-w-0 flex flex-col gap-5">
                <div className="rounded-3xl border border-slate-200 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-[0_18px_60px_rgba(15,23,42,0.24)]">
                  ...
                  <div className="px-5 py-5 overflow-y-auto max-h-[500px] custom-scrollbar space-y-6">
                    {/* User Order */}
                    <div className="space-y-3">
                       <h5 className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 flex items-center gap-2">
                          <div className="h-1.5 w-1.5 rounded-full bg-sky-500"></div>
                          Your Sequence
                       </h5>
                       <div className="space-y-2">
                          {userOrderText.map((paragraph, idx) => (
                             <div key={idx} className="flex gap-3 rounded-2xl border border-slate-200 bg-slate-50/50 p-3 dark:border-slate-700/50 dark:bg-slate-950/50">
                                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-sky-500 text-[10px] font-bold text-white">
                                   {idx + 1}
                                </span>
                                <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">{paragraph}</p>
                             </div>
                          ))}
                       </div>
                    </div>
                    ...
                    {/* Correct Order */}
                    <div className="space-y-3">
                       <h5 className="text-xs font-bold uppercase tracking-widest text-emerald-500 flex items-center gap-2">
                          <div className="h-1.5 w-1.5 rounded-full bg-emerald-500"></div>
                          Correct Sequence
                       </h5>
                       <div className="space-y-2">
                          {correctOrderText.map((paragraph, idx) => (
                             <div key={idx} className="flex gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/20 p-3 dark:border-emerald-500/20 dark:bg-emerald-500/5">
                ...
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700/70 dark:text-amber-400/70">
                      Understand the logical flow and connections between paragraphs.
                    </p>
                  </div>
                </div>
