          ...
          <div className="flex flex-wrap gap-2">
            {items.map((item, index) => (
              <span
                ...
                className="rounded-full border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900/40 dark:text-gray-200"
              >
                {item}
              </span>
            ))}
          ...
          <div className="flex items-start justify-between gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-700">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                Highlight Incorrect Words Response Analysis
              </h3>
            ...
            <div className="max-h-[70vh] space-y-6 overflow-y-auto overscroll-contain pr-1">
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                {renderWordList(
                  ...
                  "No highlighted words were recorded for this attempt.",
                )}

                {renderWordList(
                  "Incorrect Words In Audio",
                  incorrectWords,
                  <AlertTriangle className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />,
                  "Incorrect words are not available for this attempt.",
                  "success",
                )}
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                {renderWordList(
                  "Missed Incorrect Words",
                  missedIncorrectWords,
                  <Target className="h-5 w-5 text-amber-500" />,
                  "You did not miss any incorrect words.",
                  "warning",
                )}

                {renderWordList(
                  "Incorrectly Highlighted Correct Words",
                  incorrectlyHighlightedWords,
                  <XCircle className="h-5 w-5 text-rose-500" />,
                  "You did not incorrectly highlight any correct words.",
                  "danger",
                )}
              </div>
