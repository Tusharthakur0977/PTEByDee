...
const ResponseDetailModal: React.FC<ResponseDetailModalProps> = ({
  ...
  const displaySelectedAnswers = selectedTexts.length
    ? selectedTexts
                  ...
                  <div className="space-y-3">
                    <div>
                      <h3 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">
                        MCQ Single Answer (Listening) Analysis
                      </h3>
                      <p className="mt-1 max-w-2xl text-sm text-slate-600 dark:text-slate-400">
                        Review your choice and see the correct answer with detailed reasoning.
                      </p>
                    </div>
          ...
          <div className="custom-scrollbar flex-1 overflow-y-auto p-4 sm:p-6">
            <div className="grid gap-5 xl:grid-cols-2">
              <div className="min-w-0 flex flex-col gap-5">
                ...
                <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-[0_18px_60_rgba(15,23,42,0.24)]">
                  ...
                  <div className="px-5 py-5 space-y-5 overflow-y-auto max-h-[500px] custom-scrollbar">
                    ...
                    <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4 dark:border-slate-800 dark:bg-slate-950/50">
                       <div className="mb-3 flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Your Selection</span>
                       </div>
                       {displaySelectedAnswers.length > 0 ? (
                         <div className="space-y-3">
                           {displaySelectedAnswers.map((text, idx) => {
                             const isCorrect = displayCorrectAnswers.includes(text);
                             return (
                               <div key={idx} className={`rounded-xl border p-4 text-xs font-medium leading-relaxed ${
                                 isCorrect 
                         ...
                         <p className="text-xs text-slate-500 dark:text-slate-400">No selection made.</p>
                       )}
                    </div>
                    ...
                    <div className="rounded-2xl border border-emerald-100 bg-emerald-50/20 p-4 dark:border-emerald-500/10 dark:bg-emerald-500/5">
                       <div className="mb-3 flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300">Correct Answer</span>
                       </div>
                       <div className="space-y-3">
                         {displayCorrectAnswers.map((text, idx) => (
                           <div key={idx} className="rounded-xl border border-emerald-200 bg-white p-4 text-xs font-medium leading-relaxed text-emerald-700 dark:border-emerald-500/20 dark:bg-slate-900 dark:text-emerald-300">
                             {text}
                           </div>
                         ))}
                       </div>
                ...
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700/70 dark:text-amber-400/70">
                      A breakdown of why the correct option is the best fit.
                    </p>
                  </div>
                </div>
