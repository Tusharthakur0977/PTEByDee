...
const AudioRecorder = React.forwardRef<any, AudioRecorderProps>(
  (
    ...
    if (!isSupported) {
      return (
        <div className='p-4 text-sm bg-red-50 border border-red-200 rounded-lg text-red-700'>
    ...
    return (
      ...
      >
        <div className='flex items-center gap-10 justify-evenly'>
          ...
          <div className='relative group flex items-center justify-center gap-2 text-xs text-gray-600'>
            <Info className='w-4 h-4 text-blue-600' />
            <span className='font-medium text-blue-700'>Recording tips</span>

            {/* Tooltip */}
            <div className='absolute top-full mt-2 hidden group-hover:block w-72 bg-white border border-gray-200 shadow-lg rounded-lg p-3 text-xs text-gray-700 z-50'>
        ...
        <div className='dark:bg-gray-800 py-3 w-[100%] flex-1'>
          ...
          {!isRecording && !audioURL && (
            <div className='flex flex-col items-center w-full py-6 px-4 bg-gray-50 border border-gray-200 rounded-lg'>
              {/* Mic Visual */}
              ...
              {/* Helper text */}
              <p className='mt-3 text-xs text-gray-700 text-center leading-relaxed'>
          ...
          {isRecording && (
            <div className='w-full bg-red-50 border border-red-200 rounded-lg px-4 py-4'>
              {/* Top row: Mic + REC */}
          ...
          {/* Upload Progress */}
          {isUploading && (
            <div className='w-full bg-blue-50 border border-blue-200 rounded-lg px-4 py-4'>
              {/* Top row */}
              <div className='flex items-center gap-4 mb-3'>
                {/* Upload icon */}
                <div className='relative'>
                ...
                {/* Percentage */}
                <div className='ml-auto text-sm font-mono text-blue-800'>
                  {uploadProgress}%
                </div>
              </div>
          ...
          {audioURL && !isUploading && (
            <div className='w-full bg-green-50 border border-green-200 rounded-lg px-4 py-4'>
              {/* Header */}
              <div className='flex items-center gap-3 mb-3'>
                ...
                {/* Upload status badge */}
                {uploadSuccess && (
                  <div className='ml-auto px-3 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-full border border-green-200'>
                    Uploaded
                  </div>
                )}
              </div>
