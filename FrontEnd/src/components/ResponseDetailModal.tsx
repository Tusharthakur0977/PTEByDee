...
const ResponseDetailModal: React.FC<ResponseDetailModalProps> = ({
  ...
  const renderCorrectAnswer = () => {
    ...
    if (
      ...
      const choiceResult = analysis.choiceResult || {};
      const selectedAnswer =
        choiceResult.selectedTexts?.[0] ||
        analysis.selectedOptionText ||
        response.textResponse ||
        "Not selected";
      const correctAnswer =
        choiceResult.correctTexts?.[0] ||
        analysis.correctOptionText ||
        analysis.correctAnswer ||
        "Unknown";
      const explanation =
        choiceResult.explanation || analysis.explanation || null;
                ...
                {expandedError === "spelling" && (
                  <div className="border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 p-4 space-y-4">
                    {spellingErrors.map((error: any, idx: number) => (
                      <div
                        key={idx}
                        className="relative bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm pl-4 border-l-4 "
                      >
                  ...
                  <div className="border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 p-4 space-y-4">
                    {grammarErrors
                      ...
                      .map((error: any, idx: number) => (
                        <div
                          key={idx}
                          className="relative bg-white dark:bg-gray-800 p-4 rounded-lg border  dark:border-gray-700 shadow-sm pl-4 border-l-4 border-red-500"
                        >
                          <div className="flex items-start space-x-3 mb-3">
                            <div className="mt-1 shrink-0">
