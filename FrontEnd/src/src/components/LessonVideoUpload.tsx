...
const LessonVideoUpload: React.FC<LessonVideoUploadProps> = ({
  onVideoUpload,
  currentVideoUrl,
  currentVideoKey,
  className = '',
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // Upload file using the lesson video endpoint
        const response = await uploadFile(file, 'upload/lesson-video', 'video');
        ...
        setError(err.message || 'Failed to upload lesson video');
    ...
    <div className={`space-y-2 ${className}`}>
      ...
      {previewUrl && !isUploading && !error && (
        <div className='flex items-center space-x-2 text-green-600 dark:text-green-400'>
          ...
          <span className='text-xs'>Lesson video uploaded successfully</span>
        </div>
      )}
    </div>
  );
