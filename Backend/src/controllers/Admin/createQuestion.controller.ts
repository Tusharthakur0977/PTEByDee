...
export const createQuestion = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    ...
    try {
      ...
      if (questionType.name.includes('MULTIPLE_CHOICE')) {
        const optionBasedCorrectAnswers = getCorrectOptionTexts(options);
        if (optionBasedCorrectAnswers.length > 0) {
          finalCorrectAnswers = optionBasedCorrectAnswers;
        }
      }
      ...
      if (
        ...
        // Check for empty array, empty object, or null/undefined
        const isFinalCorrectAnswersEmpty =
          !finalCorrectAnswers ||
          (Array.isArray(finalCorrectAnswers) &&
            finalCorrectAnswers.length === 0) ||
          (typeof finalCorrectAnswers === 'object' &&
            Object.keys(finalCorrectAnswers).length === 0);

        if (isFinalCorrectAnswersEmpty) {
      ...
      // Store word mapping for HIGHLIGHT_INCORRECT_WORDS in correctAnswers
      if (
        autoGeneratedWordMapping &&
        questionType.name === 'HIGHLIGHT_INCORRECT_WORDS'
      ) {
        finalCorrectAnswers = {
          wordMapping: autoGeneratedWordMapping,
        };
      }
...
- Your response MUST be a valid JSON object with exactly this structure:
{
  ...
  "wordMapping": [
    { "correct": "original_word1", "incorrect": "incorrect_word1" },
    { "correct": "original_word2", "incorrect": "incorrect_word2" },
    { "correct": "original_word3", "incorrect": "incorrect_word3" },
    { "correct": "original_word4", "incorrect": "incorrect_word4" },
    { "correct": "original_word5", "incorrect": "incorrect_word5" }
  ]
}
...
- The wordMapping array MUST contain the mapping of correct (original) words to incorrect words
...
Return only the JSON object, no additional text or explanations.`;
      ...
      console.log('Parsed result:', {
        ...
        incorrectWordsCount: result.incorrectWords?.length,
        incorrectWords: result.incorrectWords,
        wordMapping: result.wordMapping,
      });
...
function generateSimpleIncorrectWords(transcript: string): {
  ...
  const wordMapping: Array<{ correct: string; incorrect: string }> = [];
