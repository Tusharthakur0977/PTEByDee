...
/**
 * @desc    Upload section video to S3
 * @route   POST /api/admin/upload/section-video
 * @access  Private/Admin
 */
export const uploadSectionVideo = asyncHandler(
    ...
    try {
      ...
      if (!uploadedFile.location) {
        return sendResponse(
          ...
          STATUS_CODES.INTERNAL_SERVER_ERROR,
          null,
          'Failed to upload section video. Please try again.'
        );
      }
      ...
      return sendResponse(
        ...
        'Section video uploaded successfully.'
      );

    } catch (error: any) {
      console.error('Upload section video error:', error);
